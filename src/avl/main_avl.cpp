...
using namespace TREE::AVL; // is this right?
...
int main(int argc, char *argv[]){
    ...
    TREE::AggregateStats avl_stats;
    avl_stats.tree_type = "AVL";
    ...
    for (int i = 0; i < files_to_process_count; ++i) {
        ...
        for(const std::string& word : words_in_doc){
            ...
            if(!word.empty()){
                avl_stats.total_words_processed ++;

                TREE::InsertResult result = TREE::AVL::insert(avl_tree, word, document_id);

                avl_stats.sum_of_insertion_times_ms += result.executionTime;
                avl_stats.total_comparisons_insertion += result.numComparisons;
            }
        }
    }
    ...
    avl_stats.total_indexing_time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(overal_indexing_end_time - overall_indexing_start_time).count()
    std::cout << "Indexing completed in " << avl_stats.total_indexing_time_ms << " ms." << std::endl;

    // updating stats nodes and height
    avl_stats.final_node_count = TREE::countNodes(avl_tree);
    avl_stats.final_tree_height = TREE::calculateHeight(avl_tree);

    // Running search in stats or search only
    if(command == "stats"){
        std::cout << "Documents indexed: " << avl_stats.num_docs_indexed << std::endl;
        std::cout << "etc: " << avl_stats. << std::endl;
        std::cout << "etc: " << avl_stats. << std::endl;
        ...
        while(true){
            ...
            TREE::SearchResult search_result = TREE::search(avl_tree, normalised_query);
